...
import { AuthorizationState, parseLecturersXml, useAuthorizationState } from '../../../../lib/utils'
...
function RouteComponent() {
  ...
  const faculties = Array.from(
    new Set(lecturers.map((lecturer) => lecturer.faculty ?? 'Other')),
  ).sort()
  ...
  const filteredLecturers =
    facultyFilter === 'all'
      ? lecturers
      : lecturers.filter(
        (lecturer) => (lecturer.faculty ?? 'Other') === facultyFilter,
      )

  // Group lecturers by faculty
  const lecturersByFaculty = filteredLecturers.reduce<
    Record<string, typeof lecturers>
  >((acc, lecturer) => {
    ...
    <div className="max-w-7xl mx-auto p-6">
      <div className="mb-8">
        ...
        <p className="text-gray-600 mb-4">
          Showing {filteredLecturers.length} lecturer
      ...
      <div className="space-y-8">
        {Object.entries(lecturersByFaculty).map(
