...
function RouteComponent() {
  ...
  const [typeFilter, setTypeFilter] = useState<'all' | 'written' | 'oral'>('all')
  const [locationFilter, setLocationFilter] = useState<'all' | 'online' | 'onsite'>('all')
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredExams.map(exam => {
          ...
          return (
            <div
              key={exam.id}
              className={`rounded-xl border ${isUpcoming ? 'bg-white border-blue-200' : 'bg-gray-50 border-gray-200'
            ...
            >
              <div className="flex justify-between items-start mb-4">
                ...
                <span className={`px-3 py-1 rounded-full text-sm ${exam.isWritten
